...
>
> Last updated: 2026-09-09 · **v6.1 — 固化混合新闻采访重剪设计规范（本地）**
>
...
## 2026-09-07 · 学习中心 Product 白底工作区（用户认可）

用户在实际学习中心确认“现在好看很多”。这套风格归属 **Soft Craft / Product**，不是第四档，也不是全站强制换底色。可视化基准：`index.html#learning-workspace`；实现参考：`jr-academy-web-zh/src/components/Pages/StudyCenterPage/StudySpace/`。当前为本地实现，认可视觉不等于全流程验收或部署。

### 视觉基线与优先级

- 学习中心等高频学习工作区采用 **白色画布 `#FFFFFF`**。营销页面仍可用暖奶油画布；禁止因旧版“奶油底默认”再次把已认可的学习中心整体改回米色。
- 深墨 `#17182F` 用于标题与底部选中项，珊瑚 `#FF6854` 用于主要行动按钮（配深墨文字），暖边 `#E9DFD2`、18–28px 圆角和轻柔阴影建立统一外框。
- 英文标题 Bricolage Grotesque、正文 DM Sans，中文思源黑体 / Noto Sans SC，系统中文字体兜底。字体优先本地加载，`font-display: swap`；品牌字体不可阻塞首屏或开发编译。
- AI 渐变是可选的局部品牌表达，不要求每屏紫色、光晕或渐变按钮。不要混回旧版棕色按钮与米色卡片体系。

### 内容决定布局，不只换卡片颜色

| 内容 | 布局与层级 | 工作区表面 token |
|---|---|---|
| 本周课程 | 班级选择、周切换、当地时区、课时列表；放首页前部。白课时卡配天空蓝左边线 | `learning.surface.schedule` = `#F3F8FC` |
| 继续学习 | 深墨重点区，清楚的继续学习入口；不虚构上次课时或进度 | `learning.surface.focus` → `color.primary.dark` |
| 探索方向 | 黄色辅助区，官方思考姿态牛小匠与一个入口 | `learning.surface.explore` = `#FFF6D9` |
| 学习记录 / 路径 | 薄荷绿区内开放列表，分隔而非层层套卡；没有进度数据就不画百分比 | `learning.surface.journey` = `#EDF7F0` |
| 我的课程 | 真实课程封面、名称、班期、资料入口；缺图用明确的书本占位，不伪造课程封面 | 白底卡片 |
| 实验室 | 技术标签 / 项目入口封面＋简短说明；示意图不能冒充实时产品截图 | 独立封面区 |
| 资料目录 | 分类、搜索和紧凑列表；公共资料、已购资料、个人收藏分清 | 白色开放内容区 |

这些 `learning.*` 是现有 Product 的语义组件变量，在 `tokens/tokens.json` 和 `tokens/tokens.css` 同步维护，不替换全局色板。不要把整个页面做成相同的“图标＋标题＋描述”卡片网格。

### 导航、插画与移动端

- **桌面与手机都用首页 / 探索 / 学习 / 实战 / 我的五项底部悬浮导航；不加桌面侧边主导航。** 页头胶囊放品牌、搜索、AI 和个人入口；手机收起次要文字。
- 底部选中项为深墨底、白字；内容必须预留 dock 高度与 safe area，不能遮住最后一个操作。点击目标至少 44px，键盘焦点可见，选中状态不能只靠颜色。
- 页面标题可配一条黄色笔刷下划线与一位官方牛小匠。复用 `assets/mascot/official/03-laptop-code-bubble.png`、`04-thinking.png`、`06-celebration.png`；同屏不混 Chibi、写实 3D 和 Career Impact 人物体系。
- 手机重新编排内容而非等比缩小桌面：单列优先、标题与插画不重叠、本周课表优先可见；实验室长说明可摘要，完整内容保留在对应入口。
- 入场 160ms、淡入＋上移 8px；hover 小幅抬升，按下反馈克制。支持 `prefers-reduced-motion`，不让装饰持续干扰阅读。

### 迁移验收

1. 中英文、390px 手机与桌面检查；首页能找到真实报名班级和本周课程。
2. 五项导航可点击，URL、刷新和前进后退状态一致；保留已有路由与权限。
3. 正常、加载、无报名、无排课、接口失败分别呈现；失败不能伪装成无课程。
4. 插画/字体可加载、无横向溢出、底部不遮挡，正文与按钮满足 AA 对比度。
5. 记录本地 / 部署状态；一次截图或 HTTP 200 不代表稳定运行及所有深层流程验收完成。

本节对学习中心的规定高于下方 v5 的“全奶油底 / 软风不用 Bricolage / 必须 AI 光晕”等历史条目。其他页面沿用对应场景的 v6 档位，不做未授权的全站迁移。

## 2026-09-07 · 插画与展示组合

交互基准：`index.html#illustration-lab`。插画方式和展示版式独立选择，外框仍遵循 Product / Editorial / Campaign，不新增 register。

| 插画方式 | 用途 | 约束 |
|---|---|---|
| 手绘人物 | 处境、情绪、成长叙事 | 一个主角和明确动作；标题用真实文字 |
| 柔和 3D | 产品总览、品牌主视觉 | 克制材质与配色；装饰不冒充真实 UI |
| 场景拼贴 | 多任务、能力组合、封面 | 同一画风，最多三个辅助贴片 |
| 结构图解 | 架构、流程、路径 | 文字、节点和连接可编辑，关系清晰 |

展示版式包括居中大图、左图右文、整幅展示。手机端图文上下排，结构图转纵向，不能把整张桌面图缩小到文字无法阅读。Product 主画布使用干净底色；Editorial / Campaign 可以使用浅网格与局部荧光笔强调。

现有示例复用 JR 资产；漫画分镜属于手绘叙事的后续扩展，未将参考截图作为自有素材收录。
...
#### A10 首页 Hero（2026-06-23 锁定：`homepage-soft.html` 当前版）

> **当前金标准 = `homepage-soft.html` hero**。首屏必须一眼传达「学 AI 来匠人」和「从诊断到作品」，不能再回到普通教育平台的 dashboard 卡片。

**结构**
- 背景：全幅沉浸式，不做左右 split card。用 `product-ecosystem-banner.png` 这类**无文字生成图 / 抽象生态图**做右侧或底部背景，叠奶油透明遮罩和浅网格，文字永远在真实 DOM 里。
- 左侧：小 badge → 强主标题 → 品牌 tagline 图 → 具体副文案 → 4 个能力 chip → 双 CTA → 3 个轻量指标。
- 右侧桌面：AI 路线诊断浮层 + 牛小匠 + 三步路线卡。它是“产品如何工作”的可视化，不是后台学习看板。
- 移动端：隐藏复杂右侧场景，只保留背景图氛围、主标题、chips、CTA、指标，并露出下一段 quick cards。首屏不能长到看不到下一个区块。

**文案**
- H1 必须是品牌/offer 本身：`学 AI 来匠人` / `从诊断到作品` 这种直接表达。
- 副文案要具体说明路径：AI 诊断背景和目标 → 接到课程/训练营/匠系工具 → 产出可展示作品。不要写“开启 AI 未来”“全面提升竞争力”这类空话。
- Hero 不承诺 offer、薪资、变现结果；只说路线、作品、能力和工具。

**视觉**
- 大标题可以用标志渐变，但同屏只允许一个 hero-scale 渐变主语。
- CTA 用标志渐变；次 CTA 用白底柔阴影。不要在 soft hero 里使用 neo 的 3px 黑边硬阴影。
- 右侧浮层是证据，不是装饰堆叠：任何浮层都要解释“诊断 / 推荐 / 落地”的一环。
- 牛小匠桌面可做视觉锚；移动端如造成首屏过长，优先隐藏角色场景而不是压小文字。

**验收**
- 桌面 1440×1000：首屏里 nav、H1、CTA、右侧路线场景、下一段 quick cards 顶部都能读到。
- 移动 390×950：H1、正文、CTA、三项指标、下一段 quick cards 至少第一张必须露出。
- 图片资源必须 200，无假字、无水印、无伪 logo；页面出错数 = 0。

#### A11 诊断 → 产品生态承接区

首页不是“课程列表”，而是“先诊断，再承接”。`homepage-soft.html` 当前产品矩阵为 Register A 标准组件。

**结构**
- 诊断器独立在 hero 之后：用户输入一句背景/目标，输出 3 步路线。
- 产品生态区紧跟诊断器：左侧解释“为什么这些产品属于同一条路线”，右侧放生态地图，下面横滑产品卡。
- 数据证明卡用当前真实数量：核心匠系工具 `3`，当前展示产品/品牌 `8`，统一入口 `1`。数量变化时先改页面再更新文档。

**生态地图**
- 中心是 JR Academy / AI Learning Hub；周围节点只放关键产品：考证匠、求职匠、牛小匠、专属路线。
- 背景图可以用生成图，但产品名、logo、节点说明必须用 DOM + 真实品牌资产。
- 地图连接线是轻量证据线，不能压住节点文字；移动端优先可读性，必要时降低背景图 opacity / 隐藏装饰 chip。

**交互联动**
- 诊断结果必须同步高亮下方产品卡和右侧生态地图节点：
  - `p1` → 考证匠 / cert
  - `p2` → 求职匠 / job
  - `p3` → 牛小匠 / uni
- 高亮态短暂存在（约 2.8s），结束后恢复，不永久污染卡片状态。
- 没有明确产品落点的路线可以只显示路线，不强行高亮。

#### A12 AI 生成配图资产治理

生成图可以进入 Register A，但只能做**无文字、无 logo、无假 UI 文案**的装饰/氛围/抽象概念图。

**允许**
- 抽象 AI 学习生态背景、空白卡片、路线光线、blank UI panels、无字 icon、soft 3D clay/glass 风格。
- 作为 hero 背景、section visual、图卡装饰、生态地图底图。
- 落盘到 `assets/illustrations/generated/`，文件名按用途命名：`product-ecosystem-banner.png`、`diagnosis-route-card.png`、`product-matrix-card.png`。

**禁止**
- 任何可读文字、数字、中文字符、伪按钮文案、伪 logo、伪产品名、水印。
- 用生成图替代真实产品 logo / 真实页面 UI / 真实用户证据。
- 绿幕抠图边缘不干净时强行上线；宁可改成奶油底图卡。

**检查**
- 生成后必须肉眼打开检查：无文字、无水印、无绿边、语义不误导。
...
#### A15 混合新闻采访重剪（2026-09-09）

本模式用于 ABC News、公司官方访谈、发布会等**真实动态素材主导**的中文新闻重剪。
它属于 Register A 的 `A-editorial` 变体：证据和人物在前，JR 图形只负责组织信息、
提高中文可读性和完成品牌收束。Harrison.ai 双画幅成片是第一份认可的视觉基准，
但其中的公司、人物、数字与新闻画面不能复用为其他事件的事实模板。

**不可变的画面语法**

- 第 0 帧就出现正在说话或自然运动的真实人物；中心问题同步出现，来源标识退居次级。
- 原声出现时始终保留来源；人物身份条使用真实 Logo、已核实姓名与职务，不制作伪 Logo。
- 新闻原声负责事实与关键表述，中文桥接只补背景、机制和边界。来源文章、官方声明、
  融资或裁员数据必须作为可读证据穿插，不能只靠装饰卡片讲述。
- 一屏只设一个视觉主角。真人、主标题、字幕、证据卡按照当下叙事功能轮换主次，
  不把所有模块同时堆成仪表盘。
- 常驻 JR 标识只保留一个，固定使用完整“匠人学院 / JR ACADEMY”官方 lockup，不得以独立 JR Box 图标代替，也不得遮挡人脸或新闻台标。左上角不再放“AI 大事件”及其说明小字。
- 结尾先在 3–4 秒真实动态画面上完成事件总结，再进入完整匠人学院 Logo 和
  “学 AI 找匠人”品牌锁定；片尾不能代替新闻结论。

**现有 token 与质感**

| 用途 | 规范 |
|---|---|
| 画布 | 奶油白 `#FFFCF6` / `#FFF7F1`，允许极淡珊瑚或紫色氛围光，不使用冷灰蓝大底 |
| 信息面 | 白色圆角卡，18–28px 圆角、1px 暖灰边 `#F1E8D8`、柔多层阴影 |
| 主文字 | 墨色 `#17182F`；正文不得使用细字重 |
| 重点 | 珊瑚 `#FB6A4A`；数据关系可少量使用 `#9B6BFF`，不得大面积渐变铺字 |
| 字体 | 中文优先思源黑体 / Noto Sans SC；英文标签可用 DM Sans；不让生成模型绘制文字或 Logo |

本节只组合现有 Register A token，不新增颜色、圆角或阴影 token。

**发布封面例外**

- 上表“不让生成模型绘制文字或 Logo”适用于视频画面、字幕、证据卡和品牌片尾。Hybrid News 的独立发布封面按 `hybrid-news-recut` Skill 的 Cover contract 执行：必须由内置 Image Model 生成完整图文栅格成品，最终简体中文标题同时在模型输出中完成，禁止生成后程序化补字。
- 封面标题必须逐字人工核对，并在可用时增加 OCR；错字、繁体、变形字、缺字或替换字一律重做。
- 官方 Logo 若进入封面，必须以真实完整 lockup 作为参考并保持准确；模型无法准确保留时宁可不放，禁止接受伪 Logo。视频画面中的常驻与片尾 Logo 仍必须使用真实品牌资产。

**9:16 视频号 / Reels 构图**

- 母版为 1080×1920；四周安全边距 36–48px，平台底部交互区额外预留。
- 真实视频通常占上部约 42–50%，完整人物优先于激进裁脸；标题、证据和总结在下部形成清晰纵向节奏。
- 主标题 72–88px；原声中文字幕 52–64px；人物姓名 32–40px；来源、职务与辅助标签 24–30px。
  手机按 270px 宽预览时，字幕、来源和人名仍必须可辨识。
- 完整匠人学院 lockup 默认宽 320px，可在 300–340px 内调整并保持原始比例；距离顶部和右侧均不得小于 36px，270px 宽手机预览时“匠人学院 / JR ACADEMY”仍须可辨识。
- 字幕容器的文字必须视觉垂直居中：上下内边距平衡，基线不得贴近容器底部；两行字幕按语义断行。

**16:9 YouTube 构图**

- 母版为 1920×1080；采访画面保持主导，通常占左侧或中央 62–72%。
- 右侧 24–30% 可作为证据轨，依次承载来源、公司背景、融资数字、原文引述或边界说明；
  同一时刻只强调一个证据模块。
- 底部字幕带建议 120–160px 高，字幕 48–60px，并以真实文本包围盒做视觉垂直居中。
- 横版必须重新排版人物、证据和字幕，不得把竖版直接裁切、缩放或加黑边。

**组件顺序与动效**

1. `Moving human hook`：真人动态开场，问题或具体变化在 0–0.5 秒进入。
2. `Source badge`：真实媒体 / 公司来源，随原声音轨保持可见。
3. `Verified lower third`：Logo + 姓名 + 已核实职务，短暂入场后降为次级。
4. `Evidence panel`：文章截图、官方声明或数据卡；必须标来源与日期，并保证正文截取可读。
5. `Number / quote emphasis`：关键数字或短引述一次只突出一个，不混用货币、轮次或估值口径。
6. `Editorial conclusion`：回到动态人物或现场，以本事件的具体总结结束。
7. `JR brand lockup`：默认复用已验收的动态片尾，显示完整匠人学院 Logo 与“学 AI 找匠人”。

- 画面保持真实运动；允许 2–4 秒内 1–3% 的轻推、轻移或重构，不用静态截图假装视频。
- 卡片以 180–260ms 淡入、上移或轻缩放进入；数字可用 8–12 帧揭示，避免持续弹跳。
- 细渐变进度线可用于串联段落，但不能比人物、字幕和证据更抢眼。
- 装饰动画必须服务于换证据、换说话人或换叙事阶段；无信息变化的循环动效应删除。

**直接打回**

- 冻结人物首帧、先放大标题或先播长 Logo 动画；
- 字幕小于上述基准、细字重、语义断行错误，或文字在字幕容器中明显偏下；
- 新闻原声还在播放却隐藏来源，或用 JR 旁白覆盖原声；
- 用大面积深蓝半透明板盖住新闻页面，堆叠多个同权重数字卡；
- 伪造媒体 Logo、人物姓名、职务、公司融资、裁员人数或发布时间；
- 左上角出现“AI 大事件”与重复说明小字；
- 结尾只剩品牌口号、Logo 被裁切，或没有完整事件总结；
- 9:16 与 16:9 共用一次自动裁切结果。

**成片验收**

- 抽检 0.2、1.2、2.2 秒，确认真人持续运动、中心问题可读、来源与人物身份无误。
- 抽检每个证据切换和三张结尾帧；确认编辑段落始终显示完整匠人学院 lockup，新闻结论完整，随后完整 Logo 与口号可读。
- 9:16 以 270px 宽手机预览复核；16:9 以 1280×720 窗口复核；字幕带均检查视觉垂直居中。
- 最终审美只认编码 MP4 的抽帧和播放结果，不以时间线、组件源码或静态设计稿代替。
- 编辑与事实工作流遵循 `.codex/skills/hybrid-news-recut/SKILL.md`；发布工作流另走
  `.codex/skills/new-media-manager/SKILL.md`。
...
_v6.1 · 2026-09-09 · JR Academy 总品牌_

_Changelog_
- **v6.1 (2026-09-09)**: 新增 **§A15 混合新闻采访重剪**，把 Harrison.ai 双画幅成片确认的真人动态首屏、来源与身份条、大号中文字幕、证据轨、双画幅独立构图、字幕视觉居中和完整匠人学院动态片尾固化为 `A-editorial` 视频规范。本次只复用现有 Register A token，不修改 `tokens.json` / `tokens.css`。
